...
import { Pill, User, Package, CheckCircle2, Clock, AlertTriangle } from 'lucide-react';
import { prescriptions, patients, medicaments, lots, centres } from '@/lib/demo-data';
  ...
  return (
    <>
      ...
      <main className="p-6">
        ...
        <div className="mb-8">
          <div className="flex items-center gap-2 mb-4">
            ...
            <h3 className="text-lg font-bold text-[var(--text-primary)]">
              En attente de dispensation ({enAttente.length})
            </h3>
          </div>
          ...
          {enAttente.length === 0 ? (
            ...
            <div className="space-y-4">
              {enAttente.map(rx => {
                ...
                return (
                  <div key={rx.id} className={`glass-card ${rx.urgence ? '!border-red-300 !bg-red-50/40' : ''}`}>
                    <div className="flex items-start gap-4">
                      {/* Patient info */}
                      <div className={`w-14 h-14 rounded-xl flex items-center justify-center flex-shrink-0 ${
                        rx.urgence ? 'bg-red-100' : 'bg-blue-50'
                      }`}>
                        <User className={`w-7 h-7 ${rx.urgence ? 'text-red-600' : 'text-blue-600'}`} />
                      </div>
                      ...
                      <div className="flex-1">
                        ...
                        <p className="text-sm text-[var(--text-secondary)] mb-3">
                          Prescrit par Dr FETY André • {rx.type_traitement} • {rx.type_saignement || ''}
                        </p>
                        ...
                        <div className="space-y-2">
                          {rx.lignes.map(ligne => {
                            ...
                            // FEFO: trouver le lot qui expire en premier
                            const lotsDisponibles = lots
                              .filter(l => l.medicament_id === ligne.medicament_id && l.quantite_restante > 0)
                            ...
                            return (
                              <div key={ligne.id} className="flex items-center gap-4 p-3 rounded-xl bg-white/50 border border-gray-100">
                                <Pill className="w-5 h-5 text-[var(--accent)] flex-shrink-0" />
                                <div className="flex-1">
                                  <p className="font-semibold text-sm">{med?.nom_complet}</p>
                                  <p className="text-xs text-[var(--text-muted)]">
                                    Quantité prescrite: <strong>{ligne.quantite_prescrite} {med?.unite}</strong>
        ...
        {/* Historique des dispensations récentes */}
        <div>
                  ...
                  return rx.lignes.map(ligne => {
                    ...
                    return (
                      <tr key={ligne.id}>
                        ...
                        <td><span className="badge badge-success"><CheckCircle2 className="w-3 h-3" />Dispensé</span></td>
                      </tr>
                    );
                  });
                })}
