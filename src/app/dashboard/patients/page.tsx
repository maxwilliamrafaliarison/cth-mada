...
import { Search, Plus, Filter, Eye, Edit, Download, Users, HeartPulse } from 'lucide-react';
            ...
            <div className="glass-card w-[380px] flex-shrink-0 animate-slide-in">
              ...
              <div className="space-y-3 text-sm">
                <div className="flex justify-between">
                  <span className="text-[var(--text-muted)]">N° CTH</span>
                  <span className="font-semibold">{selected.numero_cth}</span>
                </div>
                <div className="flex justify-between">
                  <span className="text-[var(--text-muted)]">N° WBDR</span>
                  <span className="font-mono">{selected.numero_wbdr}</span>
                </div>
                <div className="flex justify-between">
                  <span className="text-[var(--text-muted)]">Date de naissance</span>
                  <span>{new Date(selected.date_naissance).toLocaleDateString('fr-FR')}</span>
                </div>
                <div className="flex justify-between">
                  <span className="text-[var(--text-muted)]">Âge</span>
                  <span className="font-semibold">{calculateAge(selected.date_naissance)}</span>
                </div>
                <div className="flex justify-between">
                  <span className="text-[var(--text-muted)]">Poids</span>
                  <span>{selected.poids ? `${selected.poids} kg` : 'Non renseigné'}</span>
                </div>
                <div className="flex justify-between">
                  <span className="text-[var(--text-muted)]">Groupe sanguin</span>
                  <span className="font-bold text-[var(--secondary)]">{selected.groupe_sanguin || 'Non renseigné'}</span>
                </div>
                <div className="flex justify-between">
                  <span className="text-[var(--text-muted)]">Taux de facteur</span>
                  <span className="font-semibold">{selected.taux_facteur || 'Non renseigné'}</span>
                </div>
                <div className="flex justify-between">
                  <span className="text-[var(--text-muted)]">Inhibiteurs</span>
                  <span className={selected.presence_inhibiteurs ? 'text-red-600 font-bold' : ''}>{selected.presence_inhibiteurs ? 'Oui' : 'Non'}</span>
                </div>
                <div className="flex justify-between">
                  <span className="text-[var(--text-muted)]">Traitement à domicile</span>
                  <span>{selected.traitement_domicile ? 'Oui' : 'Non'}</span>
                </div>
                {selected.date_diagnostic && (
                  <div className="flex justify-between">
                    <span className="text-[var(--text-muted)]">Date de diagnostic</span>
                    <span>{new Date(selected.date_diagnostic).toLocaleDateString('fr-FR')}</span>
                  </div>
                )}
                {selected.circonstances_decouverte && (
                  <div>
                    <span className="text-[var(--text-muted)] block mb-1">Circonstances de découverte</span>
                    <span className="text-xs">{selected.circonstances_decouverte}</span>
                  </div>
                )}
                {selected.adresse && (
                  <div>
                    <span className="text-[var(--text-muted)] block mb-1">Adresse</span>
                ...
                {selected.telephone && (
                  <div className="flex justify-between">
                    ...
                    <span className="text-xs">{selected.telephone}</span>
                  </div>
                )}
            ...
            <div className="glass-card w-full max-w-3xl max-h-[85vh] overflow-y-auto !bg-white/90" onClick={e => e.stopPropagation()}>
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                <div>
                  ...
                  <input type="text" className="glass-input w-full" placeholder="Ex: 102A" />
                </div>
                <div>
                  <label className="block text-xs font-semibold text-[var(--text-secondary)] mb-1">N° WBDR</label>
                  <input type="text" className="glass-input w-full" placeholder="Identifiant WFH" />
                </div>
                <div>
                  <label className="block text-xs font-semibold text-[var(--text-secondary)] mb-1">Nom *</label>
                  <input type="text" className="glass-input w-full" placeholder="Nom de famille" />
                </div>
                <div>
                  <label className="block text-xs font-semibold text-[var(--text-secondary)] mb-1">Prénom *</label>
                  <input type="text" className="glass-input w-full" placeholder="Prénom(s)" />
                </div>
                <div>
                ...
                <div>
                  ...
                  <input type="number" className="glass-input w-full" placeholder="Poids en kg" />
                </div>
                <div>
                ...
                <div>
                  <label className="block text-xs font-semibold text-[var(--text-secondary)] mb-1">Type d&apos;hémophilie *</label>
                  <select className="glass-select w-full">
                    <option value="HA">Hémophilie A (déficit FVIII)</option>
                    <option value="HB">Hémophilie B (déficit FIX)</option>
                  </select>
                </div>
                <div>
                  <label className="block text-xs font-semibold text-[var(--text-secondary)] mb-1">Sévérité *</label>
                  <select className="glass-select w-full">
                ...
                <div>
                  ...
                  <input type="text" className="glass-input w-full" placeholder="Ex: <1%, 0.015, 0.05" />
                </div>
                <div>
                ...
                <div className="md:col-span-2">
                  ...
                  <input type="text" className="glass-input w-full" placeholder="Adresse complète" />
                </div>
                <div>
                  <label className="block text-xs font-semibold text-[var(--text-secondary)] mb-1">Téléphone</label>
                  <input type="tel" className="glass-input w-full" placeholder="034 XX XXX XX" />
                </div>
                <div>
                  <label className="block text-xs font-semibold text-[var(--text-secondary)] mb-1">Email</label>
                  <input type="email" className="glass-input w-full" placeholder="email@exemple.com" />
                </div>
                <div className="md:col-span-2">
                  <label className="block text-xs font-semibold text-[var(--text-secondary)] mb-1">Circonstances de découverte de la maladie</label>
                  <textarea className="glass-input w-full h-20 resize-none" placeholder="Ex: hémorragie labiale post-morsure, hémorragie post-circoncision..." />
                </div>
