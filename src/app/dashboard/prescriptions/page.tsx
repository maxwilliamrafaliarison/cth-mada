...
import { ClipboardList, Plus, Search, Clock, CheckCircle2, AlertCircle, User, Pill } from 'lucide-react';
import { prescriptions, patients, medicaments, centres } from '@/lib/demo-data';
import type { TypeTraitement, TypeSaignement, StatutPrescription } from '@/types';
      ...
      <main className="p-6">
        ...
        <div className="space-y-4">
          {filtered.map(rx => {
            ...
            return (
              <div key={rx.id} className="glass-card animate-fade-in">
                <div className="flex items-start gap-4">
                  {/* Avatar patient */}
                  <div className={`w-12 h-12 rounded-xl flex items-center justify-center flex-shrink-0 ${
                    rx.urgence ? 'bg-red-100' : 'bg-blue-50'
                  }`}>
                    <User className={`w-6 h-6 ${rx.urgence ? 'text-red-600' : 'text-blue-600'}`} />
                  </div>
                  ...
                  <div className="flex-1 min-w-0">
                    ...
                    <p className="text-sm text-[var(--text-secondary)] mb-2">
                      <span className="font-mono text-xs">{rx.numero}</span> •{' '}
                      {rx.type_traitement} •{' '}
                      {rx.type_saignement || 'Non spécifié'}
                    </p>
                    ...
                    {rx.autres_precisions && (
                      <p className="text-xs text-[var(--text-muted)] italic">{rx.autres_precisions}</p>
                    )}
                  </div>
                  ...
                  <div className="text-right flex-shrink-0">
                    ...
                    <p className="text-xs text-[var(--text-muted)] mt-2">
                      {new Date(rx.date_prescription).toLocaleDateString('fr-FR', { day: 'numeric', month: 'long', year: 'numeric' })}
                    </p>
                    {rx.statut === 'En attente' && (
              ...
              <div className="space-y-4">
                <div>
                  ...
                  <select className="glass-select w-full">
                    ...
                    {patients.filter(p => p.statut === 'Actif').map(p => (
                      <option key={p.id} value={p.id}>{p.numero_cth} - {p.nom} {p.prenom} ({p.type_hemophilie === 'HA' ? 'Hémo. A' : 'Hémo. B'})</option>
                    ))}
                  </select>
                </div>
                <div className="grid grid-cols-2 gap-4">
                  <div>
                    ...
                    <select className="glass-select w-full">
                      <option value="Demande">À la demande (épisode aigu)</option>
                ...
                <div>
                  <label className="block text-xs font-semibold text-[var(--text-secondary)] mb-1">Médicament prescrit *</label>
                  <select className="glass-select w-full">
                ...
                <div className="grid grid-cols-2 gap-4">
                  <div>
                    <label className="block text-xs font-semibold text-[var(--text-secondary)] mb-1">Quantité prescrite (UI/mg) *</label>
                    <input type="number" className="glass-input w-full" placeholder="Ex: 1000" />
                  </div>
                  <div>
                    <label className="block text-xs font-semibold text-[var(--text-secondary)] mb-1">Posologie</label>
                    <input type="text" className="glass-input w-full" placeholder="Ex: 500 UI x 2/jour" />
                  </div>
                </div>
                <div>
                  <label className="block text-xs font-semibold text-[var(--text-secondary)] mb-1">Précisions complémentaires</label>
                  <textarea className="glass-input w-full h-20 resize-none" placeholder="Description chirurgie, remarques..." />
                </div>
