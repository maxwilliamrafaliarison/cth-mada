...
import { Search, Plus, Package, AlertTriangle, Clock, Filter, Eye, Trash2, ArrowUpDown } from 'lucide-react';
...
export default function StockPage() {
  ...
  // FEFO: Tri par date d'expiration (premier à expirer en premier)
      ...
      <main className="p-6">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-4 gap-4 mb-6">
          <div className="glass-card !p-4 flex items-center gap-3">
            ...
            <div>
              <p className="text-2xl font-bold">{totalUnites.toLocaleString('fr-FR')}</p>
              <p className="text-xs text-[var(--text-muted)]">Unités en stock</p>
            </div>
          </div>
          <div className="glass-card !p-4 flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-xs text-[var(--text-muted)]">Lots actifs</p>
            </div>
          </div>
          <div className="glass-card !p-4 flex items-center gap-3">
            ...
            <div>
              <p className="text-2xl font-bold text-amber-600">{alertesExp}</p>
              <p className="text-xs text-[var(--text-muted)]">Proches expiration (&lt;90j)</p>
            </div>
          </div>
          <div className="glass-card !p-4 flex items-center gap-3">
            ...
            <div>
              <p className="text-2xl font-bold text-red-600">{stockFaible + expires}</p>
              <p className="text-xs text-[var(--text-muted)]">Stock faible / Expiré</p>
            </div>
          </div>
        </div>
        ...
        <div className="glass-card !p-4 mb-6">
          <div className="flex flex-wrap items-center gap-3">
            <div className="relative flex-1 min-w-[240px]">
              ...
              <input
                type="text"
                placeholder="Rechercher un médicament, n° de lot..."
            ...
            <button
              ...
              onClick={() => setSortBy(sortBy === 'expiration' ? 'stock' : sortBy === 'stock' ? 'nom' : 'expiration')}
            >
                ...
                {sorted.map(lot => {
                  ...
                  return (
                    <tr key={lot.id} className={lot.jours_avant_expiration <= 90 ? '!bg-amber-50/30' : lot.jours_avant_expiration <= 0 ? '!bg-red-50/30' : ''}>
                        ...
                        <div className="flex items-center gap-2">
                          <div className="w-16 h-2 bg-gray-100 rounded-full overflow-hidden">
                            <div
                              ...
                              style={{
                                width: `${pctRestant}%`,
                                backgroundColor: pctRestant <= 15 ? '#EF4444' : pctRestant <= 30 ? '#F59E0B' : '#10B981',
                              }}
                            />
                          </div>
                      ...
                      <td>
                        <span className={`badge ${expBadge.class}`}>{expBadge.text}</span>
                      </td>
                      <td>
