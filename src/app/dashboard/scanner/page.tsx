...
export default function ScannerPage() {
  ...
  const handleSearch = (code: string) => {
    ...
    if (lot) setResult(lot);
    else setError(`Aucun lot trouvé pour le code "${code}"`);
  };
        ...
        <div className="max-w-3xl mx-auto">
          <div className="glass-card mb-6">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-3 gap-3">
              ...
              <button onClick={() => setMode('camera')} className={`p-4 rounded-xl border-2 transition-all text-center hover:shadow-lg hover:-translate-y-0.5 duration-300 ${mode === 'camera' ? 'border-[var(--primary)] bg-blue-50/50' : 'border-transparent bg-white/30 hover:bg-white/50'}`}>
                <Camera size={32} weight="duotone" className={`mx-auto mb-2 ${mode === 'camera' ? 'text-[var(--primary)]' : 'text-[var(--text-muted)]'}`} />
                <p className="text-sm font-semibold">Caméra</p>
                <p className="text-xs text-[var(--text-muted)]">Photo du code-barres</p>
              </button>
              <button onClick={() => setMode('manuel')} className={`p-4 rounded-xl border-2 transition-all text-center hover:shadow-lg hover:-translate-y-0.5 duration-300 ${mode === 'manuel' ? 'border-[var(--primary)] bg-blue-50/50' : 'border-transparent bg-white/30 hover:bg-white/50'}`}>
                <Keyboard size={32} weight="duotone" className={`mx-auto mb-2 ${mode === 'manuel' ? 'text-[var(--primary)]' : 'text-[var(--text-muted)]'}`} />
            ...
            {mode === 'douchette' && (
              <div className="text-center">
                <div className="w-20 h-20 rounded-2xl bg-blue-50 flex items-center justify-center mx-auto mb-4">
                  <Barcode size={40} weight="duotone" className="text-[var(--primary)]" />
                </div>
                <h4 className="font-bold text-lg mb-2">Mode douchette USB</h4>
                <p className="text-sm text-[var(--text-secondary)] mb-4">Scannez le code-barres du médicament avec votre douchette USB. Le code sera automatiquement saisi.</p>
                <input type="text" className="glass-input w-full max-w-md mx-auto text-center text-lg font-mono" placeholder="Scannez ou tapez le code-barres..." value={codeInput} onChange={e => setCodeInput(e.target.value)} onKeyDown={handleKeyDown} autoFocus />
            ...
            {mode === 'camera' && (
              <div className="text-center">
                <div className="w-full h-64 rounded-xl bg-gray-900 flex items-center justify-center mb-4 relative overflow-hidden">
                  ...
                  <Camera size={64} weight="duotone" className="text-white/30" />
                </div>
                <p className="text-sm text-[var(--text-secondary)]">Placez le code-barres du médicament dans le cadre de la caméra.</p>
                <button className="btn btn-primary mt-3"><Camera size={18} weight="duotone" />Activer la caméra</button>
              </div>
            )}
            {mode === 'manuel' && (
              <div className="text-center">
                ...
                <div className="mt-3 flex flex-wrap justify-center gap-2">
                  ...
                  {lots.slice(0, 5).map(l => (
                    <button key={l.id} className="text-xs font-mono px-2 py-1 rounded bg-white/50 hover:bg-white/80 transition-colors" onClick={() => { setCodeInput(l.numero_lot); handleSearch(l.numero_lot); }}>{l.numero_lot}</button>
                  ))}
                </div>
              </div>
            )}
          ...
          {error && (
            <div className="glass-card !bg-red-50/80 !border-red-200/50">
              <div className="flex items-center gap-3"><Warning size={24} weight="duotone" className="text-red-500" /><p className="text-red-700 font-medium">{error}</p></div>
            </div>
          )}
            ...
            <div className="glass-card !bg-emerald-50/50 !border-emerald-200/50 animate-fade-in">
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                <div className="space-y-2">
                  ...
                  <div className="flex justify-between text-sm"><span className="text-[var(--text-muted)]">Indication</span><span>{resultMed.indication}</span></div>
                  <div className="flex justify-between text-sm"><span className="text-[var(--text-muted)]">Fabricant</span><span>{resultMed.fabricant}</span></div>
                </div>
                <div className="space-y-2">
                  <div className="flex justify-between text-sm"><span className="text-[var(--text-muted)]">N° de lot</span><span className="font-mono font-bold">{result.numero_lot}</span></div>
                  <div className="flex justify-between text-sm"><span className="text-[var(--text-muted)]">Quantité restante</span><span className="font-bold">{result.quantite_restante} / {result.quantite_recue}</span></div>
                  <div className="flex justify-between text-sm"><span className="text-[var(--text-muted)]">Date d&apos;expiration</span><span className={`font-bold ${joursExp <= 90 ? 'text-red-600' : ''}`}>{new Date(result.date_expiration).toLocaleDateString('fr-FR')} ({joursExp}j)</span></div>
                  <div className="flex justify-between text-sm"><span className="text-[var(--text-muted)]">Conservation</span><span>{resultMed.conservation}</span></div>
                </div>
              </div>
              <div className="flex gap-3 mt-4 pt-4 border-t border-emerald-200/50">
                <button className="btn btn-primary flex-1"><Package size={18} weight="duotone" />Ajouter au stock</button>
                <button className="btn btn-glass flex-1">Voir l&apos;historique du lot</button>
              </div>
            </div>
          )}
        </div>
