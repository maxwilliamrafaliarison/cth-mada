...
import { ArrowLeftRight, Plus, ArrowRight, Clock, CheckCircle2, Truck, Package, XCircle } from 'lucide-react';
...
const statutConfig: Record<string, { icon: typeof Clock; class: string; color: string }> = {
  ...
  'Approuvé': { icon: CheckCircle2, class: 'badge-info', color: 'bg-blue-50' },
      ...
      <main className="p-6">
          ...
          <div className="flex items-center justify-between">
            <div>
              <p className="text-sm text-[var(--text-secondary)]">
                Les centres provinciaux demandent l&apos;approvisionnement au <strong>CTH Antananarivo (centre central)</strong>.
                Le stock est déduit du centre source à l&apos;expédition.
              </p>
            </div>
        ...
        <div className="space-y-4">
          {transferts.map(trf => {
            ...
            return (
              <div key={trf.id} className={`glass-card ${config.color}`}>
                <div className="flex items-start gap-4">
                  ...
                  <div className="text-right flex-shrink-0">
                    <p className="text-xs text-[var(--text-muted)]">
                      {new Date(trf.date_demande).toLocaleDateString('fr-FR', { day: 'numeric', month: 'long', year: 'numeric' })}
                    </p>
                    {trf.statut === 'Demandé' && (
